...
Options options = {true, false, false, false, false, NULL, false, false, -1.0, -1.0};
...
int CommandlineOptionsParse(int argc, char * argv[])
{
	...
	char * short_options = "p:t:y:slqdauh";
	...
	while ((opt = getopt(argc, argv, short_options)) != -1){
		...
		switch (opt){
			case 'p':
				...
				break;
            case 't':
                options.temperature_override = true;
                options.temperature = atof(optarg);
                break;
            case 'y':
                options.proton_fraction_override = true;
